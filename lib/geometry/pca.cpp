/* This is pca.cpp and is part of StatChemLIB
...
#include "statchem/geometry/pca.hpp"
#include <assert.h>
...
string print_line(gsl_matrix* eigenmatrix, const gsl_matrix* projection,
    ...
    auto gsl_vec = gsl_matrix_column(eigenmatrix, 0).vector;
    geometry::Vector3 unit_vector(gsl_vector_get(&gsl_vec, 0),
     gsl_vector_get(&gsl_vec, 1), gsl_vector_get(&gsl_vec, 2));
    dbgmsg(unit_vector);
    ...
    const geometry::Point geom_center(gsl_vector_get(mean, 0), gsl_vector_get(mean, 1), gsl_vector_get(mean, 2));
    dbgmsg(geom_center);
